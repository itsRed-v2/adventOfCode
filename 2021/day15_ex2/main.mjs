import { input } from "../day15_ex1/input.mjs";
import PathNode from "../day15_ex1/pathNode.mjs";
import Status from "../day15_ex1/status.mjs";
import Coordinates from "../day9_ex2/coordinates.mjs";

const input_terrain = input.split("\n").map(line => line.split("").map(char => parseInt(char)));

// Resising terrain

const terrainLine = [...Array(input_terrain.length)].map(e => []);

for (let i = 0; i < 5; i++) {
	for (let row = 0; row < terrainLine.length; row++) {
		terrainLine[row] = terrainLine[row].concat(input_terrain[row].map(riskLevel => {
			riskLevel += i;
...
for (let i = 0; i < 5; i++) {
	TERRAIN = TERRAIN.concat(terrainLine.map(row => row.map(riskLevel => {
		riskLevel += i;
		if (riskLevel > 9) riskLevel -= 9;
		return riskLevel;
	})));
...
const NODES = [...Array(TERRAIN.length)].map(e => new Array(TERRAIN.length)); // 2D array with size of TERRAIN.length

let found = false;

const start = createNode(0, 0, undefined); // starting node

for (let i = 0; i < 6300000 && !found; i++) {
