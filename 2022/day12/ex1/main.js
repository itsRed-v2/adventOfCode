...
for (let x = 0; x < lenX; x++) {
	for (let y = 0; y < lenY; y++) {
		let char = map[y][x];
		if (char === "S") startPoint = newVec(x, y);
		if (char === "E") endPoint = newVec(x, y);
	}
}
...
function newPathNode(pos, parent) {
	...
	const Hcost = pos.distance(endPoint);
	...
	function getAccessibleNeighbours() {
		...
		const maxHeight = getHeight() + 1;
		return neighbours.filter(n => n.pos.isValidPos() && n.getHeight() <= maxHeight);
	}
