...
int main() {
    ...
    for (const auto &entry : wires) {
        ...
        if (wireName[0] != 'z') continue;

        cout << wireName << ": " << (entry.second == ONE ? "1" : "0") << endl;
    ...
    bitset<64> binaryValue(formedValue);
    cout << "The gates output binary " << binaryValue << ", decimal " << formedValue << endl;
}
